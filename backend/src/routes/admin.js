...
router.get('/stats', async (req, res) => {
  try {
    const [usersR, propsR, pendingR, revenueR, boostsR] = await Promise.all([
      ...
      query(`SELECT COUNT(*)::int AS count FROM properties WHERE status = 'pending'`),
      query(`
      ...
      `),
      query(`SELECT COUNT(*)::int AS count FROM boosts WHERE status = 'active' AND expires_at > NOW()`),
    ]);

    const statusCounts = {};
    propsR.rows.forEach(r => { statusCounts[r.status] = r.count; });
...
router.patch('/properties/:id/approve', async (req, res) => {
  try {
    ...
    await sendNotification({
      ...
      body: `Your listing "${rows[0].title}" has been approved and is now live.`,
      data: { property_id: rows[0].id },
    });
...
router.patch('/properties/:id/reject', async (req, res) => {
  try {
    ...
    await sendNotification({
      ...
      body: `Your listing "${rows[0].title}" was rejected. Reason: ${reason}`,
      data: { property_id: rows[0].id, reason },
    });
...
router.patch('/users/:id/role', async (req, res) => {
  try {
    ...
    if (!['user', 'dealer'].includes(role)) {
      return res.status(400).json({ success: false, message: 'Role must be user or dealer' });
    }
    ...
    if (!rows.length) return res.status(404).json({ success: false, message: 'User not found or is admin' });
...
router.patch('/dealers/:id/verify', async (req, res) => {
  try {
    ...
    if (!rows.length) return res.status(404).json({ success: false, message: 'Dealer not found' });

    res.json({ success: true, data: rows[0] });
  } catch (err) {
    console.error('Admin verify dealer error:', err);
    res.status(500).json({ success: false, message: 'Failed to toggle dealer verification' });
  }
});
...
router.patch('/reports/:id', async (req, res) => {
  ...
  try {
    ...
    if (action === 'dismiss') {
      await client.query(`UPDATE reports SET status = 'dismissed' WHERE id = $1`, [req.params.id]);
    } else if (action === 'remove') {
      await client.query(`UPDATE reports SET status = 'resolved' WHERE id = $1`, [req.params.id]);
      await client.query(`UPDATE properties SET status = 'inactive' WHERE id = $1`, [report.property_id]);
  ...
  try {
    ...
    const [countR, dataR, todayR, monthR, totalR] = await Promise.all([
      ...
      query(`
        ...
        LIMIT $1 OFFSET $2
      `, [limit, offset]),
      query(`SELECT COALESCE(SUM(amount), 0)::int AS revenue FROM payments WHERE status = 'paid' AND created_at >= CURRENT_DATE`),
      query(`SELECT COALESCE(SUM(amount), 0)::int AS revenue FROM payments WHERE status = 'paid' AND created_at >= date_trunc('month', NOW())`),
      query(`SELECT COALESCE(SUM(amount), 0)::int AS revenue FROM payments WHERE status = 'paid'`),
    ]);
...
router.post('/boost/grant', async (req, res) => {
  ...
  try {
    ...
    if (!property_id || !duration_days) {
      return res.status(400).json({ success: false, message: 'property_id and duration_days required' });
    }
    ...
    await sendNotification({
      userId: propRows[0].owner_id,
      type: 'boost_granted',
      title: 'Free Boost Granted',
      body: `Your listing has been boosted for ${duration_days} days by admin.`,
      data: { property_id, boost_id: boostRows[0].id },
    });

    res.json({ success: true, data: { boost_id: boostRows[0].id, expires_at: expiresAt } });
  } catch (err) {
    await client.query('ROLLBACK');
...
router.get('/analytics', async (req, res) => {
  try {
    const [
      byTypeR, byCityR, byMonthR, enquiriesByMonthR,
      topViewedR, userGrowthR
    ] = await Promise.all([
      query(`SELECT property_type, COUNT(*)::int AS count FROM properties WHERE status = 'active' GROUP BY property_type ORDER BY count DESC`),
      query(`
