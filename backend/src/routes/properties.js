...
const {
  ATTRIBUTE_SCHEMAS,
  PROPERTY_TYPES,
} = require("../constants/propertyAttributes");
...
router.get("/my/listings", verifyToken, async (req, res) => {
  ...
  const { rows } = await query(
    ...
    SELECT p.id, p.property_id, p.title, p.property_type, p.category,
           ...
           p.is_boosted, p.boost_expires_at,
           pl.city, pl.locality,
           (SELECT url FROM property_media pm WHERE pm.property_id = p.id AND pm.is_cover = true LIMIT 1) as cover_photo
    FROM properties p
    LEFT JOIN property_locations pl ON pl.property_id = p.id
...
router.get("/", async (req, res) => {
  ...
  if (min_price)
    conditions.push(`p.price >= ${addParam(parseFloat(min_price))}`);
  if (max_price)
    conditions.push(`p.price <= ${addParam(parseFloat(max_price))}`);
  if (possession)
    conditions.push(`p.possession_status = ${addParam(possession)}`);
  ...
  if (min_area) {
    try {
      conditions.push(
        `p.area_sqft >= ${addParam(toSqft(parseFloat(min_area), area_unit))}`,
      );
    } catch {}
  }
  if (max_area) {
    try {
      conditions.push(
        `p.area_sqft <= ${addParam(toSqft(parseFloat(max_area), area_unit))}`,
      );
    } catch {}
  }
  if (q) {
    conditions.push(
      `(p.title ILIKE ${addParam("%" + q + "%")} OR p.description ILIKE $${params.length})`,
    );
  }
  ...
  try {
    ...
    `,
      [
        ...
        price_unit || "total",
        area_sqft,
        parseFloat(area_value),
        area_unit,
        status,
        possession_status || null,
        contact_call !== false,
        contact_whatsapp === true,
        contact_enquiry !== false,
...
router.get("/:id", async (req, res) => {
  const { id } = req.params;
  const isPropertyId = /^[A-Z0-9]{5}$/i.test(id);
  const isUUID =
  ...
  if (req.user?.id) {
    ...
    if (!recentViews.length) {
      await query(
        `INSERT INTO property_views (property_id, user_id) VALUES ($1, $2)`,
        [property.id, req.user.id],
      );
      await query(
  ...
  return res.json({
    ...
    data: {
      ...
      contact_info: {
        ...
        phone: isLoggedIn ? property.owner_phone : null,
        whatsapp_url:
          isLoggedIn && property.contact_whatsapp && property.owner_phone
            ? `https://wa.me/91${property.owner_phone}`
            : null,
        show_login_prompt:
          !isLoggedIn && (property.contact_call || property.contact_whatsapp),
      },
    },
  });
});
...
router.put("/:id", verifyToken, async (req, res) => {
  ...
  allowed.forEach((f) => {
    if (req.body[f] !== undefined) {
      params.push(
        f === "attributes" ? JSON.stringify(req.body[f]) : req.body[f],
      );
  ...
  await query(
    `UPDATE properties SET ${sets.join(", ")} WHERE id = $${params.length}`,
    params,
  );
  ...
  if (req.body.city || req.body.locality) {
    ...
    const locParams = [];
    if (req.body.city) {
      locParams.push(req.body.city);
      locSets.push(`city = $${locParams.length}`);
    }
    if (req.body.locality) {
      locParams.push(req.body.locality);
      locSets.push(`locality = $${locParams.length}`);
    }
    if (req.body.pincode) {
      locParams.push(req.body.pincode);
      locSets.push(`pincode = $${locParams.length}`);
    }
    locParams.push(req.params.id);
    await query(
      `UPDATE property_locations SET ${locSets.join(", ")} WHERE property_id = $${locParams.length}`,
      locParams,
    );
  }
...
router.delete("/:id", verifyToken, async (req, res) => {
  ...
  await query(`UPDATE properties SET status = 'inactive' WHERE id = $1`, [
    req.params.id,
  ]);
