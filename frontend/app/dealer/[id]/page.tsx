        ...
        <div className="flex flex-col sm:flex-row items-start gap-6">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              <h1 className="text-2xl font-bold text-gray-900">{dealer.name || 'Dealer'}</h1>
            ...
            <div className="flex items-center gap-4 mt-3 text-sm text-gray-500">
              ...
              <span>{dealer.total_listings} active listing{dealer.total_listings !== 1 ? 's' : ''}</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {dealer.listings.map((listing: any) => (
            ...
            >
              ...
              {/* Info */}
              <div className="p-4">
                <p className="text-lg font-bold text-gray-900">{formatPrice(listing.price, listing.price_unit)}</p>
                <p className="text-sm text-gray-700 font-medium mt-1 line-clamp-1">{listing.title}</p>
                <div className="flex items-center gap-2 mt-2 text-xs text-gray-500">
                  ...
                  {listing.area_display_value && listing.area_display_unit && (
                    <>
                      <span>&middot;</span>
                      <span>{formatArea(listing.area_display_value, listing.area_display_unit)}</span>
                    </>
                  )}
                </div>
                <p className="text-xs text-gray-400 mt-2">
                  {listing.city}{listing.locality ? `, ${listing.locality}` : ''} &middot; {formatRelativeDate(listing.listed_at)}
                </p>
              </div>
            </Link>
          ))}
