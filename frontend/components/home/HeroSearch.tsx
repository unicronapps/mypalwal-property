...
export default function HeroSearch() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const { data } = await api.get("/api/search/suggest", {
          params: { q },
        });
  ...
  return (
    <div ref={containerRef} className="w-full">
      {/* Tab strip */}
      <div className="flex gap-1 mb-3 sm:mb-4 bg-gray-100/80 rounded-xl p-1 w-full sm:w-fit">
        {tabs.map((t) => (
          <button
            key={t.key}
            type="button"
            onClick={() => setTab(t.key)}
            className={`flex-1 sm:flex-none px-4 sm:px-5 py-2.5 rounded-lg text-[13px] sm:text-sm font-semibold transition-all ${
              tab === t.key
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                maxLength={5}
                className="w-full border border-gray-300 rounded-xl pl-9 pr-16 py-3 text-base font-mono tracking-widest uppercase focus:outline-none focus:ring-2 focus:ring-primary-500 placeholder:normal-case placeholder:tracking-normal placeholder:font-sans"
              />
              {propertyId && (
            ...
            <button
              ...
              className="bg-primary-600 text-white rounded-xl font-semibold px-7 py-3 hover:bg-primary-700 transition-colors disabled:opacity-40 whitespace-nowrap w-full sm:w-auto"
            >
              Find
            </button>
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              <select
                ...
                className="border border-gray-300 rounded-xl px-3 py-3 text-sm bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-primary-500 w-full sm:w-40"
              >
              ...
              <div className="relative flex-1 min-w-0">
                ...
                <input
                  ...
                  className="w-full border border-gray-300 rounded-xl pl-9 pr-9 py-3 text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-primary-500"
                  autoComplete="off"
                />
                {location && (
                  <button
                ...
                {/* Location dropdown */}
                {showSuggestions &&
                  (localMatches.length > 0 || suggestions.length > 0) && (
                    <div className="absolute top-full left-0 right-0 mt-1.5 bg-white border border-gray-200 rounded-2xl shadow-xl z-50 overflow-hidden">
                      <ul className="max-h-60 sm:max-h-72 overflow-y-auto overscroll-contain">
                        {/* ── Local localities ── */}
              ...
              <select
                ...
                className="border border-gray-300 rounded-xl px-3 py-3 text-sm bg-white text-gray-700 focus:outline-none focus:ring-2 focus:ring-primary-500 w-full sm:w-48"
              >
              ...
              <button
                type="submit"
                className="bg-primary-600 text-white rounded-xl font-semibold px-7 py-3 hover:bg-primary-700 active:scale-[0.97] transition-all whitespace-nowrap w-full sm:w-auto"
              >
                Search
              </button>
            </div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full border border-gray-200 bg-gray-50/70 rounded-xl pl-9 pr-9 py-2.5 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:bg-white transition-colors"
                autoComplete="off"
              />
              {keyword && (
                <button
                      ...
                      <button
                        ...
                        className={`text-xs px-3 py-1.5 rounded-full border font-medium transition-colors ${
                          keyword === kw
                            ...
                            : "border-gray-200 text-gray-600 hover:border-primary-400 hover:text-primary-600 active:bg-primary-50 bg-white"
                        }`}
                      >
                        {kw}
                      </button>
      ...
      {tab === "id" && (
        <p className="text-[11px] sm:text-xs text-gray-400 mt-2">
          Property IDs are shown on listing cards and shared by sellers. Each ID
          is unique to one listing.
        </p>
      )}
    </div>
  );
